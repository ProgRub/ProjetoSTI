...
class PersonsAPI {
  ...
  Future<String> addPersonIfNotInDB(String actorName, String type) async {
    ...
    try {
      print("SUMMARY");
      String summary = await getSummary(actorName);

      print("DATE BORN");
      String dateBorn = await getBirthDate(actorName);

      print("DATE DIED");
      String dateDied = await getDeathDate(actorName);

      print("AWARDS");
      var awards = await getAwardWinsAndNomination(actorName);
      var awardWins = awards["Wins"];
      var awardNoms = awards["Nominations"];

      print("PHOTO");
      String photo = await getPhotoLink(actorName);
      if (people.docs.any((element) => element["name"] == actorName)) {
        var person =
            people.docs.firstWhere((element) => element["name"] == actorName);
        if (!person["type"].contains(type)) {
  ...
  Future<String> getSummary(String actorName) async {
  ...
  Future<String> getBirthDate(String actorName) async {
  ...
  Future<String> getDeathDate(String actorName) async {
  ...
  Future<String> getPhotoLink(String actorName) async {
    var photo = "";
  ...
  Future<Map<String, int>> getAwardWinsAndNomination(String actorName) async {
    var awardWins = 0;
    var awardNoms = 0;
    ...
    return {"Wins": awardWins, "Nominations": awardNoms};
  }
}
